# Web form validator/sanitizer preventing XSS & SQL injection with rules for name, email, username, message
# With specific error messages and additional email validation (Ref: https://whoapi.com/blog/understanding-valid-email-address-formats-a-short-guide/)
...
# XSS and HTML injection patterns (regex)
DANGEROUS_PATTERNS = [
    r'<script[^>]*>.*?</script>',  # Script tags
    r'<img[^>]*onerror[^>]*>',     # Image with onerror event
    r'<img[^>]*onclick[^>]*>',     # Image with onclick event
    r'<iframe[^>]*>',               # Iframe embedding
...
def validate_full_name(name):
    ...
    # Security: Block numbers (prevents injection patterns)
...
def validate_email(email):
    """
    Validate email address format based on RFC standards and best practices.
    
    Rules: 
    - Must have @ symbol (separator)
    - Local part: alphanumeric + allowed special chars (. _ - +)
    - Domain part: valid domain name + TLD (2-63 chars)
    - Max length: 320 characters
    - No spaces, no leading/trailing special chars
    ...
    if len(email) < 6:  # a@b.co is minimum realistic
        return False, "Email address is too short", "Email must be at least 6 characters (e.g., a@b.co)"
    
    # Security: Spaces indicate manipulation attempts
    if ' ' in email:
    ...
    # Check for consecutive special characters in local part (dots, underscores, hyphens)
    ...
    if re.search(r'_{2,}', local_part):
        return False, "Email contains consecutive underscores", "Multiple consecutive underscores are not allowed in local part"
    
    if re.search(r'-{2,}', local_part):
        return False, "Email contains consecutive hyphens", "Multiple consecutive hyphens are not allowed in local part"
    
    # Validate allowed characters in local part
    # Allowed: alphanumeric, dot (.), underscore (_), hyphen (-), plus (+)
    ...
    # Each domain label (part between dots) validation
...
def validate_username(username):
    """
    Validate username with length and character restrictions.
    
    Rules: 4-16 chars, alphanumeric + underscore, cannot start with number
    """
    ...
    # Security: Leading numbers can cause SQL issues
    ...
    # Check for consecutive underscores (optional style rule)
    if '__' in username:
...
def validate_message(message):
    """
    Validate message with XSS and SQL injection threat detection.
    
    Rules: Not empty, max 250 chars, no dangerous HTML/SQL patterns
    """
    ...
    if sql_found:
        threats.append(f"SQL keywords detected: {', '.join(sql_found[:3])}")  # Show first 3
    ...
    # Explicit script tag check (most common XSS vector)
    ...
    # Event handler injection in image tags
...
def sanitize_full_name(name):
    """
    Remove invalid characters from full name and convert to Title Case.
    
    Steps: Strip whitespace, remove numbers, filter special chars, normalize spaces, title case
    """
    original = name
    ...
    # Whitelist filter: keep only safe characters
...
def sanitize_email(email):
    """
    Remove dangerous characters from email address and normalize.
    
    Steps: Strip spaces, filter non-email characters, remove consecutive dots
    """
    original = email
    ...
    # Whitelist: keep only valid email characters (including +)
    email = re.sub(r'[^a-zA-Z0-9@._+-]', '', email)
    
    # Remove consecutive dots (multiple periods in a row)
...
def sanitize_username(username):
    """
    Enforce username character rules and length limits.
    
    Steps: Filter invalid chars, strip leading numbers, enforce max length
    """
    original = username
    ...
    # Remove leading numbers (security requirement)
...
def sanitize_message(message):
    """
    Multi-layer sanitization using regex for XSS and SQL injection prevention.
    
    Security layers:
    1. Script tag removal
    2. Iframe tag removal
    3. Dangerous img tag removal
    4. Event handler stripping
    5. JavaScript protocol removal
    6. HTML entity escaping
    7. SQL keyword filtering
    8. SQL comment removal
    9. Length enforcement
    ...
    # Layer 3: Remove dangerous img tags with event handlers
    if '<img' in message.lower() and ('onerror' in message.lower() or 'onclick' in message.lower()):
        message = re.sub(r'<img[^>]*>', '', message, flags=re.IGNORECASE)
        notes.append("Suspicious image tags removed")
    
    # Layer 4: Strip event handlers (onclick, onload, onerror, etc.)
    ...
    # Layer 6: HTML entity escaping (converts <, >, &, etc.)
...
def validate_and_sanitize_form(form_data):
    """
    Main validation and sanitization function for all form fields.
    
    Uses dictionary to represent submitted form data (as per requirements).
    """
    # Dictionary to store validation and sanitization results
    results = {
        'validation': {},
        'sanitized': {},
        'errors': [],
    ...
    # Validate and sanitize Message (includes threat detection)
